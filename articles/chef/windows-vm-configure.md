...
![Chef アーキテクチャ](media/windows-vm-configure/chef-architecure.png)
...
お使いのワークステーション上に、最新バージョンの [Azure CLI](/cli/azure/install-azure-cli?view=azure-cli-latest) をダウンロードしてインストールします。
...
## <a name="configure-chef-server"></a>Chef サーバーの構成
...
![[Create organization]\(組織の作成\) ウィンドウ](media/windows-vm-configure/create-organization.png)

組織を作成したら、スターター キットをダウンロードします。

![Chef の構成](media/windows-vm-configure/configure-chef.png)
...
### <a name="configure-a-chef-workstation"></a>Chef ワークステーションを構成する
...
![Knife ファイルの例](./media/windows-vm-configure/knife-file-example.png)
...
![knife-azure のインストールからの出力](./media/windows-vm-configure/install-knife-azure.png)
...
## <a name="create-a-chef-cookbook"></a>Chef クックブックを作成する
...
## <a name="upload-the-chef-cookbook-to-the-chef-server"></a>Chef サーバーに Chef クックブックをアップロードする
...
![プロビジョニング中の仮想マシン](./media/windows-vm-configure/virtual-machine-being-provisioned.png)

次にコマンド プロンプトが表示されます。

![仮想マシン作成時の Knife 出力](./media/windows-vm-configure/knife-output-when-creating-vm.png)
...
![仮想マシンのテスト](./media/windows-vm-configure/vm-test.png)

この例では、クリエイティブ HTML コードを使用します。

また、[Chef Manage](https://manage.chef.io/) 上でノードのステータスも閲覧できます。

![ノードの状態の表示](./media/windows-vm-configure/view-node-status.png)
